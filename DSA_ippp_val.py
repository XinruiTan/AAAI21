...
parser.add_argument('--gain', default=0.8, type=float, 
...
baseline_time = 1.0
avg_b_time = np.linspace(2, 24, 12)
avg_b_time = np.append(avg_b_time, 25) / 25.0
...
def main_worker(args):
    ...
    train_cor = np.load('./experiment_data/DSA_val_correct.npy')
    train_cfd = np.load('./experiment_data/DSA_val_confidence.npy')

    val_cor = np.load('./experiment_data/DSA_test_correct.npy')
    val_cfd = np.load('./experiment_data/DSA_test_confidence.npy')
    ...
    np.save('./experiment_data/DSA_ippp_train_acc', acc_train)
    np.save('./experiment_data/DSA_ippp_train_delay', delay_train)
    np.save('./experiment_data/DSA_ippp_val_acc', acc_val)
    np.save('./experiment_data/DSA_ippp_val_delay', delay_val)
