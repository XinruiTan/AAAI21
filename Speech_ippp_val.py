...
parser = argparse.ArgumentParser(description='IPPP validation on LSTM-DSA-19')
parser.add_argument('--gain', default=0.4, type=float, 
...
parser.add_argument('--k', default=200, type=float, 
...
baseline_time = 1.0
avg_b_time = np.linspace(6, 96, 16)
avg_b_time = np.append(avg_b_time, 99) / 99.0
...
def main_worker(args):
    ...
    # cor = np.load('./experiment_data/SCAN_val_correct.npy') 
    # cfd = np.load('./experiment_data/SCAN_val_confidence.npy')

    # print(cor[0])
    # print(cfd[0])

    train_cor = np.load('./experiment_data/Speech_val_correct.npy')
    train_cfd = np.load('./experiment_data/Speech_val_confidence.npy')

    val_cor = np.load('./experiment_data/Speech_test_correct.npy')
    val_cfd = np.load('./experiment_data/Speech_test_confidence.npy')

    f_list = [heuristic, ippp_start, ippp]
    # f_list = [heuristic, ippp_start, ippp, dp_solver, ippp]
    # f_list = [ippp_start, ippp, dp_solver, ippp]
    # threshold = -0.889 * np.ones((15,))
    ...
    np.save('./experiment_data/Speech_ippp_train_acc', acc_train)
    np.save('./experiment_data/Speech_ippp_train_delay', delay_train)
    np.save('./experiment_data/Speech_ippp_val_acc', acc_val)
    np.save('./experiment_data/Speech_ippp_val_delay', delay_val)
